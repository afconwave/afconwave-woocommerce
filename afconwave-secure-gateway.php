 ...
 *
 * @package           AfconWaveSecureGateway
 ...
 * Description:       Securely accept Mobile Money and Card payments in Africa via the AfconWave gateway.
 * Version:           1.0.0
...
add_action('woocommerce_blocks_loaded', function () {
    ...
    require_once plugin_dir_path(__FILE__) . 'includes/class-wc-afconwave-blocks-support.php';
    add_action(
        'woocommerce_blocks_payment_method_type_registration',
        function ($payment_method_registry) {
            $payment_method_registry->register(new WC_AfconWave_Blocks_Support());
        }
    );
});
...
{
    $settings_link = '<a href="' . esc_url(admin_url('admin.php?page=wc-settings&tab=checkout&section=afconwave')) . '">' . esc_html__('Settings', 'afconwave-secure-gateway') . '</a>';
...
function afconwave_pay_shortcode_handler($atts)
{
    ...
    if (floatval($atts['amount']) <= 0) {
        return '<p style="color:red;">' . esc_html__('Invalid payment amount.', 'afconwave-secure-gateway') . '</p>';
    }
    ...
    <form method="POST" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="afconwave-pay-form">
        ...
        <button type="submit" class="button afconwave-pay-btn"
            ...
            <?php printf(esc_html__('Pay %s with AfconWave', 'afconwave-secure-gateway'), esc_html($atts['amount'] . ' ' . $atts['currency'])); ?>
        </button>
    </form>
    <?php
    return ob_get_clean();
...
function afconwave_handle_standalone_pay()
{
    if (!isset($_POST['afconwave_nonce']) || !wp_verify_nonce(sanitize_key(wp_unslash($_POST['afconwave_nonce'])), 'afconwave_pay_nonce')) {
        wp_die(esc_html__('Invalid security token.', 'afconwave-secure-gateway'), esc_html__('Error', 'afconwave-secure-gateway'), array('response' => 403));
    }
    ...
    if ($amount <= 0) {
        wp_die(esc_html__('Invalid payment amount.', 'afconwave-secure-gateway'));
    }
    ...
    if (empty($secret_key)) {
        wp_die(esc_html__('Payment gateway is not fully configured.', 'afconwave-secure-gateway'), esc_html__('Error', 'afconwave-secure-gateway'), array('response' => 500));
    }
    ...
    if (is_wp_error($response)) {
        wp_die(esc_html__('Payment error: ', 'afconwave-secure-gateway') . esc_html($response->get_error_message()));
    }
    ...
    wp_die(esc_html__('Failed to initialize payment.', 'afconwave-secure-gateway'));
}
